...
function optionChanged(sampleID) {
	...
	Plotly.d3.json("http://127.0.0.1:5000/samples/"+sampleID, function(error, response) {
		...
		console.log(otus_ids.slice(0,10));
		console.log(sample_vals.slice(0,10));
		updatePieChart(otus_ids.slice(0,10),sample_vals.slice(0,10));
...
function updatePieChart(otus_ids,sample_vals) {
	var PIE = document.getElementById("pieChart");
        Plotly.d3.json("http://127.0.0.1:5000/otu", function(error, response) {
                ...
                Plotly.restyle(PIE, "values", [sample_vals]);
                Plotly.restyle(PIE, "labels", [otus_ids]);
        });
};
...
function updateBubbleChart(otus_ids,sample_vals) {
	...
	Plotly.d3.json("http://127.0.0.1:5000/otu", function(error, response) {
		...
		Plotly.restyle(BUBBLE,"x",[otus_ids]);
		Plotly.restyle(BUBBLE,"y",[sample_vals]);
		Plotly.restyle(BUBBLE,"text",[otuLabels]);
		Plotly.restyle(BUBBLE,"marker.size",[sample_vals.map(d=>d+10)]);
		Plotly.restyle(BUBBLE,"marker.color",[sample_vals]);
	});
};

function gaugeChart(levelVal) {
        // Trig to calc meter point
        var degrees = 180 - (180./13.)*(levelVal+1);
        var radius = .5;
